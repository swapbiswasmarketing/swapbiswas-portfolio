...
If the name change also moves your market position, that is a separate decision, settled long before this plan starts. I have written up the difference in [branding vs positioning](/blog/branding-vs-positioning/). This post assumes the strategy is signed off and the date is already in the calendar.
...
![The T-90 to T+30 rebrand countdown showing five phases with named owners for each workstream](/assets/blog/rebranding-rollout-plan/countdown.webp "The T-90 to T+30 rebrand countdown")
...
**App store names are capped.** App Store Connect requires an app name of "at least two characters and no more than 30 characters", subtitle capped at 30 as well ([Apple](https://developer.apple.com/help/app-store-connect/reference/app-information/)). Google Play is equally hard-capped: "Your app title must be 30 characters or less", a short description has to land "in 80 characters or less", and "A full description allows for 4,000 characters" ([Play Console Help, store listing best practices](https://support.google.com/googleplay/android-developer/answer/13393723)). A long descriptor after the new name survives neither store.
...
The developer-surface line is the one I would put on a wall. Salesforce renamed Pardot to Marketing Cloud Account Engagement years ago, yet the Metadata API type is still called PardotSettings, described in the docs as representing "Marketing Cloud Account Engagement settings in your Salesforce org" ([Salesforce Developers](https://developer.salesforce.com/docs/atlas.en-us.api_meta.meta/api_meta/meta_pardotsettings.htm)), and the current API documentation still sits under a path carrying the old name ([Account Engagement API](https://developer.salesforce.com/docs/marketing/pardot/overview)). A rename reaches marketing in a week and the system layer in years. Write down which instances you are consciously choosing not to fix.

Your [brand guidelines](/blog/what-are-brand-guidelines/) document should end with this inventory attached, because guidelines without a surface list are a style guide nobody can execute against.
...
![Rebrand comms matrix showing four audience tiers with channel, sender and lead time before the public announcement](/assets/blog/rebranding-rollout-plan/comms-matrix.webp "The rebrand comms matrix")
...
The tiering discipline is the same one I use for any commercially sensitive announcement. If you have run a [price increase announcement](/blog/price-increase-announcement/), you already have the account tiers and the owner map. Reuse them rather than rebuilding them.
...
If the name is not yet registered, that timeline sits outside the countdown. USPTO examination "may take a number of months", and after publication "it can take three to four months" before the next official status ([USPTO](https://www.uspto.gov/trademarks/basics/trademark-process)). Start it a year out. If the new name is not locked yet, settle it with [the naming gates](/blog/product-naming/) before the countdown starts, because a trademark conflict discovered at T-30 is not a slip you can absorb.
...
On the sales side, rebuild first what reps actually open in a live call: the [competitive battlecard](/blog/competitive-battlecard-template/) set, and whatever sits on your [sales enablement checklist](/blog/sales-enablement-checklist/) as the standard first-meeting kit.
...
**The redirect map.** If the rebrand moves you to a new domain or subdomain, submit a Change of Address in Search Console. Google notes "you only need this tool when moving from one domain or subdomain to another". Map every indexed URL one to one rather than pointing everything at the homepage, and keep the redirects "for as long as possible, generally at least 1 year". Google sets the timing expectation too: "a small to medium-sized website can take a few weeks for most pages to move, and larger sites take longer" ([Google Search Central](https://developers.google.com/search/docs/crawling-indexing/site-move-with-url-changes)). Build the map at T-60, sign it off at T-14, and crawl-and-diff it the way you would in a [technical SEO site audit](/blog/how-to-conduct-a-technical-seo-site-audit/). Check the status code that comes back on the diff rather than the one in your ticket, because [a permanent redirect your host emits is often a 308, not the 301 you wrote down](/blog/301-redirect-seo/): two lines of vercel.json on this site return 308 on every permanent redirect, and nothing in the config asks you to choose.
