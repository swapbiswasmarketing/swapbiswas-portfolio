...
- [Keyword backlog](project_keyword_backlog.md) - data-backed low-KD blog opportunities. **BOTH KEYWORD TOOLS ARE DEAD**: Ahrefs sits at 399,955 / 400,000 units (resets 2026-09-21) and Semrush is out. But **`mcp__ahrefs__public-domain-rating-free` costs ZERO units**, so the whole winnability gate (sub-DR-40 proof page + top-three test) still runs free on live SERPs. **The new binding constraint is the WebSearch session budget: 200 calls, SHARED with every subagent** - ten probe agents exhausted it in one round. Fallback SERP retrieval is WebFetch against Brave (rate-limits after ~4 queries) or search.yahoo.com via curl; almost every other engine blocks this IP. Written: 7 posts 2026-07-19 + 7 on 2026-07-25 + 5 on 2026-08-15 + 10 on 2026-08-26 + 5 on 2026-08-29 + 4 on 2026-08-31 + 2 on 2026-09-01 + 1 on 2026-09-03 + **4 on 2026-09-07** (design-partner-vs-beta-customer, pricing-tier-names, striking-distance-keywords, developer-community-strategy). **THE BACKLOG IS EXHAUSTED**: the 2026-09-07 cycle gated ~50 of ~200 candidates across three rounds and exactly ONE cleared the top-three bar. Do not work down a stored list next cycle. Read the KD-TRAP rule and the newer **top-three bar** (GSC: 6.98% CTR at positions 1-3 vs 0.048% at 4-10) before picking anything - the dominant kill is now a sub-DR-40 page that plateaus at position 5-8, which is a ceiling and not a path. New rules: **the Google-Help-wall pre-screen** (if Google publishes a dedicated Help article for the exact term it is dead; cross-network only won because it is a row inside a bigger table), **Google autocomplete as a free demand check** (suggestqueries.google.com killed a term with a page-1 sub-DR-40 page and zero real demand), and **search for the ANGLE not just the keyword** (two candidates died to Teresa Torres and Orbit Media already owning the exact differentiator). Dedup greps MUST be hyphen-tolerant (`foo[ -]bar`) and MUST cover `/tools` pages. **Best optimize target on the site: developer-marketing.md** - "devrel vs developer marketing" passed every SERP test (a DR-9 page ranks page 1) but was correctly dropped because that pillar already owns the intent. Corpus health finally hit 1-in-4 first-hand on 2026-09-07. Upgrade questions are in repo `FIRST_HAND_UPGRADES.md`.
