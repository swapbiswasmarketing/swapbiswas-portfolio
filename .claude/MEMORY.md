...
- [Keyword backlog](project_keyword_backlog.md) - data-backed blog opportunities and the winnability gate. **THE NEW-POST WELL IS DRY: the 2026-09-10 cycle gated 37 candidates across three rounds and ZERO cleared the bar** (0 agent errors, so it is a real result, not a failure). Do not plan another discovery batch before Ahrefs resets **2026-09-21**; both keyword tools were dead all cycle (Ahrefs 399,955/400,000, Semrush out). **Free stack that replaced them: `mcp__ahrefs__public-domain-rating-free` costs ZERO units** so the sub-DR-40 gate still runs free, and **Google autocomplete via curl** (`suggestqueries.google.com/complete/search?client=firefox&hl=en&gl=us&q=`) is the demand oracle - an empty children array is a decisive free kill. WebSearch is the binding constraint at 200/session SHARED with subagents, so cap each agent explicitly. **START EVERY CYCLE BY MINING GSC, NOT KEYWORD TOOLS**: `.gsc/exports/gsc-queries-*.csv` cross-referenced against the corpus showed every high-impression uncovered query is a near-synonym of a page that already exists - seo report 29,394 impr/0 clicks at pos 21-58, seo audit cost 11,909 impr/3 clicks at pos 13-17, seo ranking 8,224 impr/0 clicks. That is optimize work, not new URLs. Written: 7 posts 2026-07-19 + 7 on 2026-07-25 + 5 on 2026-08-15 + 10 on 2026-08-26 + 5 on 2026-08-29 + 4 on 2026-08-31 + 2 on 2026-09-01 + 1 on 2026-09-03 + 4 on 2026-09-07 + **3 on 2026-09-10** (positioning-vs-messaging, feature-adoption-rate, developer-experience-survey - all explicit position-5 bets on the user's call, plus 3 optimize passes on what-is-seo-report, how-much-does-an-seo-audit-cost, how-do-i-check-my-seo-ranking). Standing rules: the **top-three bar** (GSC 6.98% CTR at pos 1-3 vs 0.048% at 4-10), the **KD trap**, the **reframe trap** (4 of 7 verdicts died because a reframe was never searched on its own SERP - make it an explicit prompt rule), hyphen-tolerant dedup greps covering `/tools`, and the Google-Help-wall pre-screen. **A citation that resolves is not a verified citation** - two fabricated stats were found in live opening hooks this cycle (a G2-sourced '95% of new products fail' contradicted by peer-reviewed 30-49%, and a '91% of product marketers' figure that appears nowhere on the cited PMA page), and one cited source domain now 301s to a gambling link farm, so follow redirects before shipping. **`scripts/assign-blog-covers.cjs` rewrites `img` on ~16 existing posts** when you add a batch - check `git diff --name-only` and revert cover-only churn. Best optimize targets left: `do-google-reviews-help-seo` (6,542 impr, pos 54.8) and `what-does-chatgpt-stand-for` (9,164 impr, pos 15.6). Upgrade questions are in repo `FIRST_HAND_UPGRADES.md`.
