...
- [Keyword backlog](project_keyword_backlog.md) - data-backed low-KD blog opportunities (Semrush); 7 posts written 2026-07-19 (GTM engineer, PMM career path, win-loss questions) + 7 more 2026-07-25 (product-marketing-interview-questions, sales-enablement-strategy, how-to-become-a-product-marketing-manager, customer-enablement, saas-pricing-models, content-marketing-framework, brand-messaging-framework - all built/fact-checked, reciprocal-linked, review due 2026-08-15); backlog "write next" tier depleted again; footprint log (all 7 opened problem-led - vary next) + review ledger feed the `/swapblog-optimize` loop
...
- [Concept gallery pipeline](reference_concept_gallery_pipeline.md) - add + thumbnail a new concept at /personal-website-examples/ (now 45; +Exchange/IDE/Passport/Arcade); counts derive from concepts.length; primed-hidden JS reveals need a safety-net timeout or they show empty/0
